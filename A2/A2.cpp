...
class Process {
public:
    int id, at, bt, ct, tat, wt, rt, rmt, priority;
    void input(vector<Process>& p, int n, bool isPriority);
    ...
    void output(const vector<Process>& p, int n);
...
int main() {
    try {
        ...
        switch (choice) {
            case 1: pManager.FCFS(processes, n); break;
            case 2: pManager.SJF(processes, n); break;
            case 3: pManager.RoundRobin(processes, n, tq); break;
            case 4: pManager.PriorityScheduling(processes, n); break;
        }
